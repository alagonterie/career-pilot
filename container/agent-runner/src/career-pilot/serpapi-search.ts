...
export interface GoogleJobPayload {
  ...
  /** How direct `apply_url` is (§24.190). `aggregator` also means the company
   *  attribution is unverified — Google's `company_name` is least reliable on a
   *  reposted listing. */
  apply_link_kind: ApplyLinkKind;
  title: string;
  company: string;
  company_domain: null;
...
// ── apply-link directness ranking (§24.190) ──────────────────────────────────
//
// Google returns `apply_options` in ITS order, which is not a directness order.
// Taking `[0]` blindly meant a posting that happened to surface via an
// aggregator got stored with the aggregator's URL — and those routinely demand
// an account before they'll show you the actual application. Worse, an
// aggregator repost is also where Google's `company_name` is least trustworthy
// (observed: a listing filed under one company whose description body named a
// different one entirely), so the same signal that picks the link also tells us
// how much to trust the company attribution.
//
// The ranking ALLOW-LISTS the good rather than deny-listing the bad. Aggregators
// are an endless, churning long tail (a deny-list rots the week you write it);
// applicant-tracking systems are a short, stable, enumerable set. So a host we
// recognize as an ATS — or as the hiring company's own site — wins, and
// everything else is residual. Being wrong is cheap and self-correcting: an
// unrecognized ATS just ranks as `unknown` and still beats nothing.

/** Host suffixes of applicant-tracking systems / company-careers platforms. A
 *  link on one of these goes straight to the employer's own application. */
...
/** How direct a lead's apply link is — persisted so the owner surface can flag
 *  it and the agent can caveat the company name (§24.190). */
...
/** Company-name → a comparable token ("Socket.dev" → "socket", "Acme Corp" → "acmecorp"). */
...
/** Does this host look like it belongs to the hiring company itself? Compares the
 *  registrable-ish label against the company token, so `careers.socket.dev` and
 *  `socket.dev` both match "Socket.dev" while `jobleads.com` does not. */
function isCompanyHost(host: string, company: string): boolean {
  const token = companyToken(company);
  if (token.length < 3) return false; // too short to match on without false positives
  return host
    .split('.')
...
/**
 * Pick the most direct apply link available, preferring the company's own site,
 * then a known ATS, then whatever is left. Pure + exported for tests.
 *
 * `fallback` (Google's `source_link`/`share_link`) is considered alongside the
 * apply options rather than only as a last resort — it is frequently the most
 * direct link of the lot.
 */
export function pickApplyLink(
  options: SerpApiAppplyOption[] | undefined,
  company: string,
  ...
  // Nothing recognizable anywhere => this posting only reached us through a
  // reposter. Say so, rather than leaving it indistinguishable from a direct one.
...
export function normalizeGoogleJob(job: SerpApiJob, now: Date = new Date()): GoogleJobPayload | null {
  ...
  // §24.190: rank for DIRECTNESS instead of inheriting Google's ordering. The old
  // code took `apply_options[0]`, which is whichever source Google listed first —
  // often an aggregator that gates the real application behind a signup.
  const company = job.company_name ?? '';
  const picked = pickApplyLink(job.apply_options, company, job.source_link || job.share_link || null);
  const source_url = picked.url || job.source_link || job.share_link || '';
  ...
  return {
    ...
    source_url,
    apply_url: picked.url ?? source_url,
    apply_link_kind: picked.kind,
    title,
    company,
    company_domain: null,
    location_raw,
