...
This persona is the behavioral contract for the owner agent. "Done" means
the agent's actual behavior matches what's described here. The verification
plan, in increasing rigor:

1. **Voice red-team (manual, ~30 min).** Paste 10 conversational scenarios
   covering the spectrum: routine status update, ambiguous request, agent
   mistake, candidate venting after rejection, mid-process disagreement,
   irreversible action approval, quiet-hours edge case, onboarding flow.
   Agent's responses should match the voice rules (no sycophancy, no
   apology theater, brief, peer register) and the autonomy gradient (right
   action class for each scenario). Failures get filed as either spec gaps
   (persona needs refinement) or implementation bugs (code/wiring drift).

2. **Autonomy gradient compliance (manual + log review).** For each of the
   four action classes, exercise at least one tool/action. Check the funnel
   event log + Telegram thread: "just do" actions land silently, "notify
   after" actions emit one-liners, "confirm before" actions present
   approval cards and wait, "refuse" actions return brief refusals without
   moralizing.

3. **Proactivity calibration (1-2 weeks shadow run).** During Phase 9
   shadow mode, observe whether the agent's unprompted messages pass the
   "would the candidate be glad I interrupted?" bar. Track: briefings per
   day, frequency cap hits, quiet hours violations (should be zero unless
   critical). Adjust persona if pattern is off.

4. **Sanitization-awareness check (automated, Phase 3+).** When the agent
   produces output bound for `record_funnel_event` payloads, the
   sanitization pipeline should rarely have non-trivial work to do — if
   the regex/LLM pass is consistently rewriting agent output, the persona
   isn't internalizing "write as if there's no sanitization."

5. **Reflection loop quality (qualitative, Phase 4+).** After 5 rejections,
   read the resulting `learnings` rows. They should have substantive
   `what_didnt` / `next_time` content, not boilerplate. If reflections are
   shallow, the prompting templates here need work.

This DoD is itself part of the spec. When the persona changes meaningfully,
this section gets updated too — the verification target moves with the
intent.
