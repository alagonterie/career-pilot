...
| Credential vault | **OneCLI** (NanoClaw default) for non-LLM creds (Google OAuth, Telegram, Cloudflare, etc.) | Two vaults, defense-in-depth: **Portkey's AI Provider holds the raw Anthropic key; OneCLI holds the Portkey key** (injects `x-portkey-api-key` for `api.portkey.ai`) — the container sees neither raw key. |
| Domain pattern | `hire.<DOMAIN>` (Worker) + `api.hire.<DOMAIN>` (Tunnel, **Worker-only**) | Sub-sub-domains work. **The browser talks ONLY to the Worker** (`hire.<DOMAIN>`); the Worker proxies `/api/*` — JSON *and* SSE — to the tunnel, authenticating with a Cloudflare Access **service token**. `api.<DOMAIN>` stays the tunnel ingress but is reached only by the Worker, never browser-direct. **Reverses the original "Tunnel direct for SSE" choice** (STRATEGY §24.39 D12, 2026-06-04): browser-direct meant cross-origin Access (separate cookie → every call 302'd → "offline"), and the premise that Workers can't hold SSE was disproven (no hard duration limit; `: ka` keepalive beats the 100 s idle timeout). Same-origin = one Access app, one cookie, no CORS, no priming. |
...
| Subagents | Six: `research-company`, `tailor-resume`, `draft-outreach`, `build-interview-kit` (replaces `prep-interview` per §24.53, shipped 2026-06-08), `scrape-jobs`, `pipeline-scribe` (added Phase 4 as `funnel-curator`, §24.9; renamed per §24.59 — historical audit rows keep the old `agent_name`, display-aliased). `draft-outreach` is the only writer to an *externally-visible* surface — a reversible Gmail draft via the orchestrator's `create_gmail_draft` MCP tool (per STRATEGY.md §24.3; no approval gate since drafts don't send; the future `send_outreach_email` will be approval-gated per PORTAL.md §6.3). `scrape-jobs`, `pipeline-scribe`, and `build-interview-kit` are subagent-owned writers of *durable, private* state: `job_leads` rows / the pipeline-state read-model via `persist_pipeline_state` / a per-interview kit Google Doc in the candidate's own career-account Drive via `persist_interview_kit` (an external API but a private, reversible artifact — `drive.file` scope, not approval-gated; §24.53). `research-company`, `tailor-resume` are strictly read-only. | Sandbox group has the first three only (`research-company`, `tailor-resume`, `draft-outreach`). Both groups run on NanoClaw's upstream `bypassPermissions` provider; sandbox isolation comes from `disallowedTools` bare-name removal (sandbox excludes `create_gmail_draft` — no Gmail OAuth in sandbox OneCLI scope) + `maxTurns`/`maxBudgetUsd` caps + per-thread session isolation. See AGENT_SDK_PATTERNS.md §6 for the security-layer model. |
