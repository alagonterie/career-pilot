...
| Subagents | Five (mostly read-only): `research-company`, `tailor-resume`, `draft-outreach`, `prep-interview`, `scrape-jobs`. `draft-outreach` performs one reversible external write — Gmail draft creation via the orchestrator's `create_gmail_draft` MCP tool (per STRATEGY.md §24.3). No approval gate (drafts don't send); the future `send_outreach_email` tool will be approval-gated per PORTAL.md §6.3. The other four remain strictly read-only. | Sandbox group has the first three only. Both groups run on NanoClaw's upstream `bypassPermissions` provider; sandbox isolation comes from `disallowedTools` bare-name removal (sandbox excludes `create_gmail_draft` — no Gmail OAuth in sandbox OneCLI scope) + `maxTurns`/`maxBudgetUsd` caps + per-thread session isolation. See AGENT_SDK_PATTERNS.md §6 for the security-layer model. |
