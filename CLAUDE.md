...
| Subagents | Six: `research-company`, `tailor-resume`, `draft-outreach`, `prep-interview`, `scrape-jobs`, `funnel-curator` (added Phase 4, §24.9). Only `draft-outreach` writes to the external world — a reversible Gmail draft via the orchestrator's `create_gmail_draft` MCP tool (per STRATEGY.md §24.3; no approval gate since drafts don't send; the future `send_outreach_email` will be approval-gated per PORTAL.md §6.3). `scrape-jobs` and `funnel-curator` are *internal* writers (durable backend state: `job_leads` rows / the funnel-state read-model via `persist_funnel_state`). `research-company`, `tailor-resume`, `prep-interview` are strictly read-only. | Sandbox group has the first three only (`research-company`, `tailor-resume`, `draft-outreach`). Both groups run on NanoClaw's upstream `bypassPermissions` provider; sandbox isolation comes from `disallowedTools` bare-name removal (sandbox excludes `create_gmail_draft` — no Gmail OAuth in sandbox OneCLI scope) + `maxTurns`/`maxBudgetUsd` caps + per-thread session isolation. See AGENT_SDK_PATTERNS.md §6 for the security-layer model. |
