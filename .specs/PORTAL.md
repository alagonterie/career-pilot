...
> - **Stat-line stability** — the two **count** segments are **SSR-seeded** (`getHeroSeed` fetches `/api/funnel` + `/api/telemetry` server-side, the same tunnel + Access path as `getWorkProfile`), so they're in the first-paint HTML instead of popping in from a skeleton. The relative **"last activity X ago"** stays client-only — it can't SSR without a hydration mismatch (server vs client `now`) — and appends after mount into a **fixed-height** slot (1 line desktop / 2 lines mobile) so neither the hooks taking over nor that append ever shifts the page. The skeleton shows only when the SSR seed was empty (backend unreachable at SSR). (Superseded the earlier skeleton+min-height try, which still nudged on the segment-by-segment client fill.)
