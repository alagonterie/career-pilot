...
> **Build note (§24.135 — the `/` flare pass).** A first-impression polish over the hero + section rhythm (the todo "more flavor on /"). Wider, unified viewport spacing; a CSS-only staggered entrance that rides ONLY the live chrome (badge → CTAs → stat line) while the SSR'd name/title/hook stay solid from first paint (a `motion/react` fade would blank the no-JS hero, and fading the headline undercuts the §24.71/§24.36 instant-paint); a **hero-only** ambient aurora (two faint brand-hue radial washes — deliberately NOT the ops register, where a colored wash fights the dense "Bloomberg gut" aesthetic + the AA-contrast margin); two section-heading `InfoTip`s ("My job search, live", "Agent activity") via a new `align="text"` prop that drops the §24.86 caps-nudge for normal-case labels; and the pitch transition reworded off its leading "and". A hero count-up and an above-the-fold live-action line were both considered and dropped/declined (hydration+width instability; clean-hero crowding). Home visual baselines re-blessed. Full rationale: STRATEGY §24.135.
...
> **Build note (STRATEGY §24.188 — "searching since" becomes owner-settable).** The hero's second stat segment was derived from the earliest application's `applied_at`, which quietly assumes the search has run *continuously* since then. After a deliberate pause that reads as months of inactivity — the opposite of what the segment exists to convey. It is now **owner-set first, derived second**: `candidate_profile.searching_since` (a bare `YYYY-MM`, edited on the `/admin` Persona tab) overrides the derivation, and an unset field is exactly the previous behavior. Like `site_lifecycle` (§24.149 D3), it is a claim about the search that must be owner-stated, never auto-inferred — the agent cannot set it (it isn't a `PROFILE_FIELDS` onboarding step). It rides the **pipeline read-model** for the same reason the lifecycle does: that's the one endpoint both the SSR hero seed and the client hook already fetch, so the override can't reach one and not the other and shift the line on hydration. The omit-when-empty honesty rule is unchanged — no applications *and* no override still means the segment is simply absent.
...
> **Build note (STRATEGY §24.35 Pass D).** The owner asked whether the simulator's resize-on-run is intentional — it is: running widens `main` from `max-w-2xl` → `max-w-6xl` and reveals the two panes, the deliberate Apple→ops register switch (above), a one-time transition on Run; the panes are height-bounded (no in-run jitter), so it's left unchanged — unlike the pipeline board's content-jitter, which was a real layout bug (fixed in §5.4 / Pass D).

---

### 5.4 `/pipeline` — the pipeline race detail (visitor label: **Job Pipeline**)

> **Naming (owner call, 2026-06-10; supersedes "Momentum", 2026-06-03; see STRATEGY §24.59).** Route + visitor label = **Job Pipeline** / `/pipeline` (with a redirect from the old `/momentum`, `?app=` preserved). "Momentum" was the gamified horse-race framing but failed instant-understandability; "Job Pipeline" says what the page is, and the "Job" prefix disambiguates from CI/CD pipelines for a dev audience. **As of §24.152 the *internal* plumbing is "pipeline" too** — `/api/pipeline`, `public_pipeline_view`, the `Pipeline*` components, `pipeline_events`, the `pipeline_scribe_*` config keys, and the `pipeline-scribe` subagent. The only residual "funnel" *in the system* is migration-source / frozen-history (the historical migrations + the `reconcileLegacySeriesIds` rename clause). The older build-notes below predate that rename and still cite the original `funnel_*` artifact names (`/api/funnel`, `public_funnel_view`, `funnel_events`, the funnel-curator) as the record of when each shipped — read them against today's `pipeline_*` equivalents; a full prose sweep of PORTAL's domain-term usage is remaining §24.151 work.
...
**Layout:** Ops register. The full pipeline as a horse-race style horizontal board:
...
> **Build note (per STRATEGY §24.57).** The drawer is deep-linkable: `/pipeline?app=«application_ref»` opens that card's panel once the pipeline data loads (an unknown ref is a no-op; closing the panel clears the param). The `/live` Recent-outcomes rows link here — a static outcome list becomes navigation into the drawer that already existed.

> **Build note (STRATEGY §24.153 — owner item 1, the curator signature).** The board is the `pipeline-scribe` subagent's curated output, so it is *signed* like every other AI-authored surface: the page footer carries the canonical `AgentMark actor="pipeline-scribe" lead="Curated by"` (✦ provenance, §24.73) — a signature at the foot, not a header badge. Shown only when the board has content (loading / error / ≥1 application — the §24.149-L1 handle-legend gate); the cold-start empty state stays signature-free (nothing is curated yet; the "warming up" line owns that moment).
...
> **Backend note.** The richest source for "sanitized recent activity" is the pipeline-scribe's per-company narratives (`pipeline_scribe_output`, already captured privately). Surfacing them is V1-scoped but **built in Phase 6** alongside this panel and **gated on the Pass 3 LLM sanitization review** (STRATEGY.md §24.12): the narratives are free-form prose where regex + exact-name redaction isn't sufficient. Until then this panel renders from the structured `pipeline_events` timeline + `public_pipeline_view`.
...
> **Owner-only surface — `/admin` control-center (STRATEGY §24.138 / §24.136 D1–D3).** Not a public portal surface and not in the nav: a tabbed operator cockpit (Overview · Pipeline · Visitors · Contacts · System) in the brand language, reached by direct URL. **Gating (`adminEnabled`):** open on the dev stack (already owner-Access-gated, the `/dev` trust model), **fail-closed elsewhere** — `/admin*` + `/api/admin/*` 404 until the owner wires the prod `/admin` Cloudflare Access app (owner email only) AND flips `admin_api_enabled` (the origin-JWT-validated host belt). `noindex`. It exposes **every operationally-meaningful config lever** (the include-by-default knob audit — the recruiter-sim/dev/self-referential-gate/boot-identity keys are the short, explicit deny-list) plus the prod-safe reads (health · 24 h cost · container pool · funnel · the §24.74 attribution browser · the §24.121 contacts store) and the mode controls (LIVE/SHADOW, pause/resume, kill-switch — destructives confirm-gated). The panels build + verify NOW on dev `/admin`; only the prod Access app is cutover-gated (Phase B).

> **Build note (STRATEGY §24.187 — batch lead cleanup).** The Leads tab's §24.173 triage surface was single-lead only, which makes clearing a stale pool a dozens-of-clicks job. It now carries a **batch layer**: an *older than N days* filter (over `first_seen_at` — the primitive the filter bar was missing, and the one that expresses "everything from before I paused") plus a bulk bar that acts on **exactly the rows the filters are showing**. Two actions: **Archive N** (the default — bulk soft-close, history and any lead→application link intact) and **Delete N** (a genuine purge; confirm-gated and it skips promoted leads rather than orphaning an application's provenance). Both are **two-step in place** — the first click arms the button into `Confirm … N?` with a cancel, no modal — and any filter change disarms, so an armed batch can never fire against a set the owner didn't read. The batch posts an explicit **id list**, never a filter spec the server re-derives: WYSIWYG is the safety story, so the count in the button is always the count in the table.

> **Build note (STRATEGY §24.190 — lead links carry a provenance flag).** A lead's expanded detail now states how direct its apply link is. `✓ direct company link` / `✓ direct ATS link` when the posting resolves to the employer's own site or a real applicant-tracking system; **`⚠ aggregator link · company unverified`** when it doesn't. The warning deliberately carries *two* claims because they share one cause: an aggregator-only lead may put a signup wall between the owner and the application, **and** its company name is untrustworthy — a reposted listing is exactly where the sourced company name is least reliable (observed: a lead filed under one company whose description body named another). The surface flags rather than guesses: inferring the "real" company from body text is the kind of inference that produces confident wrong answers, and a wrong company name would propagate into scoring and potentially the public board.

> **Build note (STRATEGY §24.191 — tailor a résumé to a lead, from the lead).** The Leads detail disclosure gains a *tailored résumé* strip: a download link when one exists, `Tailor résumé` / `Regenerate` otherwise, and the run's cost and age. It mounts with the disclosure, so nothing is fetched until a row is expanded. Three deliberate surface choices. (1) **The JD is a paste box, not a read-only echo of what we scraped.** The stored description is least trustworthy exactly on the leads flagged by §24.190 — and at the moment the owner reaches for this, they have the real posting open in another tab. Blank means "use what we scraped"; anything typed wins. (2) **Whether the summary was actually tailored is stated on the surface, not left in a log.** The honesty guardrail silently floors the bio back to the master when it's a stub or cites a figure that appears nowhere in the master; on a public demo that's a quality nit, but on a résumé about to be attached to a real application it's the difference between *tailored* and *my master résumé with the bullets reordered* — so it reads `✓ summary tailored` or `⚠ summary fell back to master`, the latter explaining which floor tripped. (3) **A generation in flight replaces the button rather than disabling it**, because the server has already claimed the run with a `pending` row — a refresh, a second tab, or an impatient double-click joins the existing run instead of paying for a second one. Regenerating keeps prior versions; the strip shows the count. Owner-only end to end: the trigger, the poll, and the PDF all live under `/api/admin/*`, since a résumé naming a real employer is precisely what the anonymization model exists to keep off public surfaces.
...
> **Build note (STRATEGY §24.120 — the chef's-kiss pass).** `/contact` was functionally complete but read as a generic form bolted onto the site — the one surface not participating in the "always-on agent system" thesis. Four honest, on-brand touches: (1) an **"Open to offers" live-pulse pill** (the shared `AvailabilityBadge`, reused from the home hero — pulses while the SSE feed is connected) atop the page so the sink breathes like the rest of the site; (2) the **instant-to-phone hook** — the intro + success state surface the true, delightful fact that a submission reaches the candidate's phone in seconds (it's a host-delivered Telegram relay); (3) an **elevated Sent moment** — the flat card becomes a warmer, lightly-animated (reduced-motion-safe) "✓ Sent — that just pinged my phone." (supersedes the "Sent. …replies within 24 hours." copy below); (4) the **error copy** now points at the footer socials, not the "reach me directly below" block removed in §24.83. **Agent-awareness of contacts is a separate slice (§24.121 — recall-on-demand: persist + a `read_contacts` tool, zero public-form LLM spend).**

> **Build note (STRATEGY §24.121 — contact recall + abuse backstops).** Supersedes the "not persisted to the DB" note above: contacts now **persist privately** (`contact_submissions`) so the orchestrator can recall them via the owner-only `read_contacts` tool (the owner asks "how should I reply to that Acme contact?" → an owner-initiated turn pulls it; the public form never triggers LLM spend). Because persistence is a new junk-row vector, the host relay adds defense-in-depth behind the §24.70 edge (per-IP RL + Turnstile): a **kill switch** (`contact_relay_enabled`), **dedup** (identical delivered submission within a window → no re-notify), a **global flood cap** (N persisted/window → UNAVAILABLE), and **bounded retention** (prune to the newest N). `/contact` itself spends no money (pure relay), so the only harms a flood poses are owner-Telegram spam + DB rows — both now bounded. Prod-cutover: the real Turnstile keys must be wired at launch (else bot-open behind only the per-IP RL).
...
### 7.7 `standby` — the deliberate hibernation (STRATEGY §24.189)

Everything in §7.2–7.4 is an **incident** control: something is wrong, stop it, and the portal degrades to an apologetic *"temporarily offline — back shortly."* **Standby is the opposite in both intent and mechanics.** The search is deliberately not running, the compute is genuinely gone rather than idling, and the public page says so as a *career statement* — not an outage.

| | `/halt` | `standby` |
|---|---|---|
| Trigger | something is wrong | a planned pause in the search |
| VM | keeps running (and billing) | **stopped** (~$25–30/mo → ~$5/mo) |
| Public `/` | "⏸ temporarily offline — back shortly" | "I'm not actively looking right now" + a way to reach the candidate |
| Control lives | on the host | **at the edge** — it has to, the host is off |
| Recovery | `/resume` | one click, then a health probe gates the site coming back |

**The control plane is at the edge, and that's the whole design constraint.** With the VM stopped there is no host, no DB, no tunnel — so the flag cannot be a `system_modes` row like every other mode; that table is on the disk being powered down. It lives in **Workers KV**, read by the Worker on every request, and written by an **edge console at `/admin/standby`** that talks to KV and GitHub Actions and never to the backend. The console is owner-gated for free: the prod Access app already covers `hire.<apex>/admin` *and everything under it*, enforced before the Worker runs.

**One page, not a half-live site.** `/work`, the footer socials and `/contact` are all SSR'd from the backend, so keeping them "up" with the origin gone would mean the committed `Jane Doe` placeholder and a contact form posting into the void. Standby therefore renders **one deliberate page at every public route** (exempting only the console and the static legal pages). It stays honest about identity because entry snapshots `/api/profile` into KV *while the box is still up* — so the recruiter who lands mid-standby still gets the real name, email and socials.

**Coming back is gated on being actually back.** Entry sets the flag *first* (the site switches over instantly, nobody sees a half-dead site); exit clears it *last* — only after the VM is up, overdue proactive work has been rolled forward to its next occurrence, and a health probe through the tunnel returns 200.

Row for the §7.5 table:

| Host state | Portal `/` | `/live` | `/simulator` | `/contact` |
|---|---|---|---|---|
| `standby` | Standby page: *"I'm not actively looking right now"* + direct email | Standby page | Standby page | Standby page (email link, no form — nothing to relay through) |
...
> **Version chip (STRATEGY §24.139 / §24.136 D4 — the sanctioned narrow return of the SHA, A2).** The foreshadowed "small fast-follow" lands here: a **single muted version token** next to the "Built with" credit — `v1.0.0` (prod, a link to `/releases/tag/<v>`) / `dev · <sha>` (dev, a link to the commit) / `dev` (local, no link). It is the product version (semver from `v1.0.0`, CHANGELOG-driven), **not** `package.json`'s NanoClaw-fork number, and **not** a resurrection of the retired metadata block — it carries *only* the version, no live status/cache/cost. Byte-stable and deterministic under the @visual seed (renders `dev`, no SHA in the baseline). The chip's value is build-time `VITE_APP_VERSION`/`VITE_APP_VERSION_REF`; the href composes from the `REPO_URL` single source.
...
> **Build note (2026-06-19 — unified chrome framing; supersedes the §24.82 `max-w-4xl` header note in §8.1 and the "register-aware styling" sentence above).** The header nav (§8.1), connective rail (§8.4), and footer (§8.2) now read as **one framed chrome band**. The motivating bug: each element's `border-t`/`border-b` divider was sized to a different width (the page content vs. a fixed per-register width), so the rules near the foot didn't line up and the footer *resized between pages* as you navigated. Resolution: **(1) full-bleed borders** — every chrome rule spans the viewport edge-to-edge (like the header's existing `border-b`), so they're identical on every page and never resize; **(2) one shared content gutter** — all three center their content in `CHROME_WIDTH` (`max-w-6xl`, the widest page's content width), a single constant exported from `lib/site.ts` and consumed by header + rail + footer, so the page is bracketed top-and-bottom on one column (flush with content on the wide ops pages; the narrower pages' content sits centered within it). **In-page section rules stay content-width** — they're content, not chrome — a deliberate two-tier read rather than a failed near-match. **The rail is one height everywhere** (`py-8 text-sm`: it's the conversion band, so it gets presence + comfortable tap targets, not ops-density). **The mono/terminal treatment** (mono font + uppercase label, on both rail *and* footer) is scoped to the two "system" surfaces — **`/dashboard` + `/architecture` only** — via a shared `isMonoSurface(pathname)` predicate; `/pipeline` and the rest read clean (the per-register mono coupling, and the footer's `register` prop, are retired — mono is now pathname-based). Both rail and footer read the **committed** path (`resolvedLocation`, not the pending `location`) so their config/treatment never previews the next page mid-navigation. The whole framing is one tunable: change `CHROME_WIDTH` to retune all three at once.
