...
7. **Reduced-motion is a structural guarantee, not per-component discipline** (STRATEGY §24.36 36.4). Two animation systems, two complementary mechanisms: a global `@media (prefers-reduced-motion: reduce)` reset in `app.css` neutralizes *all* CSS animation/transition (skeletons' `animate-pulse`, the stream `LiveCursor`, the ●live dot, any future one), and a single root `MotionConfig reducedMotion="user"` makes *all* motion/react animations (the grow-modal, the funnel card layout) respect it — so neither system can regress a new animation past the guarantee.
