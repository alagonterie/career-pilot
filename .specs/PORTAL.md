...
1. **Direct contact** — submits the contact form or DMs Jane via a surfaced channel.
2. **Forward up** — sends the link to their engineering hiring manager / EM / staff engineer with a positive framing.
3. **Pipeline pull** — adds Jane to their pipeline for a specific open role.

The portal succeeds when **technical hiring managers who see this page conclude, within 60 seconds of digging in, that Jane ships real systems**. Recruiter conversion is downstream of that conclusion.
...
| **Engineering hiring manager / Staff engineer** | Lands on `/`, scans hero, clicks through to `/live` and `/architecture`, opens GitHub repo in new tab, returns to `/contact` if convinced | Personal email to Jane, or recruiter handoff |
...
| **The person being shown the link by Jane** | Direct deep link to a specific page he sends them | Whatever the conversation calls for |
...
**Purpose:** In 5 seconds of viewing, the visitor learns three things: (a) who Jane is and what he does, (b) that this site is *itself* the proof, (c) where to click to go deeper or convert.
...
Tagline below: *"This page costs Jane ~$ZZ/day to run. Cache saves the rest."*
...
**What this is:** Proof-by-demonstration. A visiting recruiter or hiring manager doesn't have to take Jane's word that the system works — they type in their own company name and role description, click `Run`, and watch the same agent stack that's running his real job search execute on *their* data in real time. Within 20-30 seconds they have a tangible, downloadable artifact (tailored bullets + cold outreach email).
...
**Purpose:** The gamified deep-dive into Jane's job search. Recruiter sees motion ("this person is in demand"), engineer sees a real pipeline tracker.
...
TRIGGERS ────────────────────►  │  HOST (Node)                            │
                                │                                          │
   Telegram (Jane)   ──►   │  [Channel Adapters ●]                    │
   Portal sandbox (web)   ──►   │       │                                  │
   Gmail / Calendar       ──►   │       ▼                                  │
...
- **TRIGGERS** — what can wake the system: chat input from Jane, sandbox visitors, Google Workspace webhooks, the cron sweep.
...
**Sections:**
1. **Bio** — 2 paragraphs, voice-of-Jane
...
5. **Writing / talks** (optional, if Jane has any)
...
- **Telegram** (deep link to a public bot username — replies route to Jane via NanoClaw)
- **Email** (`mailto:` link)
- **LinkedIn**

When submitted, the message is relayed to Jane via Telegram. Sender gets a confirmation: *"Sent. Jane typically replies within 24 hours."*
...
>
> - **Portkey Model Catalog** holds Jane's Anthropic API key as a vaulted Integration. The container makes Claude calls to `api.portkey.ai` with only a Portkey API key; Portkey looks up the right Anthropic credential, makes the actual API call, and logs the trace for observability.
...
>
> A developer who forks the repo gets the system, then populates their own `candidate_profile` via the Telegram onboarding flow. None of Jane's personal content is committed.
...
|---|---|---|---|
| **Owner (Jane)** | Telegram (v1); Discord later | `career-pilot` | Full. Owner role via `user_roles`. Real DB writes, Gmail/Calendar OAuth, real outreach. |
...
| **Contact-form visitor** | `/contact` (web POST) | (none — webhook handler) | One-way relay. No conversation. Submission is delivered to Jane's channel as a system message. |
...
3. **The reflection prompt:** Within 1 hour (or at the next quiet-hours boundary if it's late), the agent posts a card to Jane on Telegram:
...
7. **Optional portal publication:** Jane can flip `reflection_published: true` per learning. Published reflections show on the application's `/funnel` detail panel as a "What I learned" block, with the company still obfuscated unless `public_state = 'public'`.
...
Career Pilot has weight — it touches real applications, real people, real money, and Jane's career. Three control mechanisms keep it safe: a system-mode flag, three pause/halt tiers, and the autonomy gradient from §6.3.
...
**The flip:** Jane promotes to `LIVE_MODE = true` via a Telegram command requiring two-step confirmation. Going back to dry-run is one command (no confirmation) — easy to back off.
...
- Cloudflare Worker serves a static "This system is paused for review. — Jane" page; backend API responses replaced with 503s.
...
| `/killswitch` active | Static page: `Paused for review — Jane` | Same | Disabled | Disabled |

The transparent "*here's why we paused*" message Jane can set is itself a credibility move — recruiters reading *"paused due to traffic spike at 5,200 RPS — diagnosing now"* see operational maturity, not breakage.
...
```
  example.com    /live    /simulator    /funnel    /work    /contact
```
...
```
  ───────────────────────────────────────────────────────────────────────
  example.com          ● SYSTEM STATUS: ONLINE          last deploy: a3f4c1
                         · 91% cache · $0.84/day
...
Jane toggles this per application via Telegram (`@career-pilot reveal stripe`) or via an admin route. **Default is `obfuscated` until explicitly toggled.**

### Rules

1. **Active applications are obfuscated by default.** Real names only after explicit flip.
2. **Closed-with-offer / hired:** Public by default IF the company has agreed (Jane confirms).
3. **Closed-rejected / withdrawn:** Stays obfuscated unless Jane chooses to publish (e.g. "Big Tech Co rejected me at the final round, here's what I learned" — only if he wants that publicly).
...
3. **LLM context-sensitivity pass (optional, async):** Haiku reviews the sanitized text for leak risk; if flagged, escalates to Jane for approval before publication.
...
| `POST /api/contact` | Relays to Jane's Telegram via NanoClaw | 1 message | <2s |
...
## 12. Content variables (TBD inputs from Jane)

Things Jane needs to provide before the portal can ship — **but the system ships without them**. See "Placeholder strategy" below for how.
...
The portal **must** ship and run without all of these populated. The user (Jane) should be able to deploy on day 1 with everything empty and see the system running end-to-end with placeholder content, then refine variables one at a time over time.
...
Jane can fill these in natural-language via Telegram at any time. Examples:
...
2. **Anonymization for hired companies that haven't agreed:** What's the policy if Jane signs an offer with a company that wants to keep his hire quiet for now? Recommendation: default `public_state = 'partial'` for any offer/hire until explicit reveal.
...
- A blog / writing CMS — link out to wherever Jane writes.
