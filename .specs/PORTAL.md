...
| `/live` (§5.2) | **Trace-first.** The live trace stream leads (the "agent working now" wow is immediately visible, not buried); the stat panels (system status, sessions, container pool, telemetry, cost, recent outcomes) stack below. **All panels kept** — honest and complete. Each trace line **stacks its summary onto its own full-width line** on a phone (a clean sentence below the metadata row, not a squeezed right-gutter wrap); the desktop single-row terminal layout is restored at `sm+`. The home live-activity ticker (§5.1) does the same (full text, not truncated). |
