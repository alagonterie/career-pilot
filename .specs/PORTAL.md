...
> - **Stat-line stability** — a **fixed height** (1 line desktop, 2 lines mobile) plus a readiness gate (the two deterministic polls settling) so the skeleton→text swap lands in one shot and never shifts the page vertically (superseded the earlier min-height try, which still nudged on the segment-by-segment fill).
