...
- **Strategy B — Persona as a host-injected fragment.** Modify the
  ...
  fork). Tracking deviation for a future manual upstream sync.
...
for a future manual upstream sync. The change is additive (a new
...
saying e.g. "/add-telegram" and rewrite NanoClaw config. A manual upstream
sync (diff against the upstream tag, re-apply our deviations) is how we'll
...
- **Symptom (2026-05-26).** `CLAUDE.md` root + decision memory + `STRATEGY.md` all said
  "pin v0.3.150", but the vendored pin was `^0.2.128` — a major version older.
- **Resolution (2026-06-19, RESOLVED — option (a)).** Bumped to
  `^0.3.170` (resolves 0.3.181) in the NanoClaw v2.1.17 upgrade
  (STRATEGY.md §24.128) — so the specs (written against 0.3.150) are now
  *aligned* with the shipping SDK. The breaking-change cost was small
  (one silent parser break: `rate_limit_event` became a top-level message
  type, fixed). The SDK now runs its own bundled, version-matched Claude
  Code binary (Path B; `pathToClaudeCodeExecutable` omitted, the separate
  claude-code install retired). Caret on a 0.x version resolves to
  `0.3.x` only. See the **Decisions made → Δ2 / Δ13** entries below for the
  full delta + the upgrade's deliberate deviations.
...
2. **Δ2 — Agent SDK version: BUMPED `^0.2.128` → `^0.3.170` (resolves 0.3.181) in the NanoClaw v2.1.17 upgrade (2026-06-19, STRATEGY.md §24.128).** The Phase-5+ revisit this Δ originally deferred is now done. Spec realigned (Stage E) — `decision_architecture` memory + `AGENT_SDK_PATTERNS.md` §0/§1 + root `CLAUDE.md` updated. 0.3.x deltas absorbed: `rate_limit_event` → top-level message type (the one silent parser break, fixed); `task_notification` now carries `status`/`usage`; `getContextUsage()`/`startup()` available; MCP startup non-blocking by default (`alwaysLoad:true` on turn-1-critical servers). See **Δ13** for the upgrade's deliberate deviations.
3. **Δ3 — Persona placement strategy:** **Strategy B — composer extension. IMPLEMENTED.** `src/claude-md-compose.ts` extended to discover `.claude-host-fragments/*.md` and include them in the composed import list. `src/container-runner.ts` mounts the dir RO. Persona content moved to `groups/career-pilot/.claude-host-fragments/persona.md` with the output-protocol section added. `.gitignore` updated for composer-managed `CLAUDE.md` + PII-bearing `candidate.md`. Old `groups/career-pilot/CLAUDE.md` removed (composer owns that path now). First deliberate deviation from upstream NanoClaw — flagged in commit message for a future manual upstream sync.
4. **Δ4 — Outbound-DB hot-journal race remediation:** **IMPLEMENTED.** The host's active delivery poll opens `outbound.db` readonly every cycle. The container writes that file in `journal_mode = DELETE`, which creates a transient `outbound.db-journal` per write transaction. If our readonly `prepare()` races inside that ~1-5ms window, SQLite trips on the journal-rollback determination and returns `SQLITE_READONLY_ROLLBACK` (extended code 776, canonical message "attempt to write a readonly database"). The 5s `busy_timeout` doesn't apply — this isn't BUSY, it's a readonly determination impossibility. Self-heals on the next 1s poll, but produces a noisy ERROR log every time. Patch: `src/delivery.ts` now exports `isReadonlyRollbackError()` and wraps the `getDueOutboundMessages` call in `drainSession` to swallow `SQLITE_READONLY_*` codes (debug log + early return — the next `ACTIVE_POLL_MS` tick absorbs). 5 unit tests for the discriminator. Fourth deliberate deviation from upstream NanoClaw — flagged in the commit message for a future manual upstream sync; upstream issue to be filed (see commit body for the draft text) so this deviation can be dropped once accepted.
5. **Δ11 — Session-topology fork deviations (2026-06-12, STRATEGY.md §24.67):** three deliberate deviations land with the ops/chat session split — flag each in commit messages for a future manual upstream sync. (a) `container/agent-runner/src/db/messages-in.ts` `getPendingMessages`: upstream applies `ORDER BY seq DESC LIMIT N` *before* filtering rows already consumed (acked in `processing_ack`), so ≥N stale pending rows permanently hide any older due row — the §24.66 outage shape. Patched to filter acks first, then cap to the newest N. Upstream-issue-worthy on its own. (b) `src/session-manager.ts` `writeSessionRouting`: thread ids with the reserved `internal:` prefix write `thread_id: null` into `session_routing`, so a host-created synthetic session (our ops session) never leaks its synthetic id to a channel adapter as a reply thread (`chat-sdk-bridge.deliver` uses `threadId ?? platformId` verbatim). (c) `src/container-runner.ts`: the career-pilot series bootstraps move out of the spawn path (to a host-sweep MODULE-HOOK targeting the ops session) and ops spawns get per-class rotation env pushed from `getConfig()` (`CLAUDE_TRANSCRIPT_ROTATE_BYTES`/`_AGE_DAYS`, `NANOCLAW_CONVERSATIONS_DIR`).
6. **Δ13 — NanoClaw v2.1.17 upgrade deviations (2026-06-18/19, STRATEGY.md §24.124–§24.130):** several deliberate divergences land with the surgical dep-bump — flag each for a future manual upstream sync. (a) **pnpm-workspace** (`pnpm-workspace.yaml`): `container/agent-runner` + `frontend` are declared host workspace members (upstream is config-only, no `packages:`) so one root `pnpm install` populates all trees and `pnpm -r typecheck` covers them — but a `container/agent-runner` dep change must update BOTH `bun.lock` (container runtime) AND the root `pnpm-lock.yaml` (host) or the deploy's `setup.sh --frozen-lockfile` fails (§24.128.1, learned the hard way). (b) **Ported the upgrade-marker tripwire** (§24.126) rather than inheriting upstream's `[BREAKING]` boot guard — our `src/index.ts` is heavily customised; `src/upgrade-state.ts` + the boot gate + the bootstrap stamp + RECOVERY.md §3.5 are our port. (c) **Path B executor** (§24.128): we omit `pathToClaudeCodeExecutable`, so the 0.3.x SDK runs its OWN bundled, version-matched Claude Code binary (`@anthropic-ai/claude-agent-sdk-<platform>` = CLI 2.1.181) — and we **retired the Dockerfile's separate `@anthropic-ai/claude-code` install** (upstream keeps it). Parity is automatic; one fewer ~200 MB CLI. (d) **`cli-tools.json` not adopted** (§24.129): upstream moved global-CLI pins into `cli-tools.json` + `install-cli-tools.sh`; we keep Dockerfile ARGs — down to one CLI (`agent-browser@0.27.1`) after pruning the unused `vercel` install (another divergence: upstream keeps vercel; we deploy via Cloudflare). (e) **OneCLI gateway on `/v1` + SDK 2.2.1** (§24.127): the host `@onecli-sh/sdk` bump required migrating the dev gateway to a `/v1`-serving image; the master-key-in-`app-data` hazard + the recovery runbook are in RECOVERY.md §3.6.
7. **Δ12 — Stop install containers on graceful shutdown (2026-06-17, STRATEGY.md §24.91):** upstream `shutdown()` tears down channels/polls but leaves dockerd-owned containers running (`KillMode=process`, and `control-group` wouldn't reach them either) — they orphan every deploy until the next boot's `cleanupOrphans()` reaps them, spending tokens unsupervised in between. **Review confirmed there is no warm-container reconnect to preserve:** resume is via the persisted SDK continuation (transcript replay), and the wake "already running" guard is the in-memory `activeContainers` Map (cleared on process start), not the DB `container_status` — so stopping on shutdown is lossless. Deviation: extract `stopInstallContainers(reason)` in `src/container-runtime.ts` (shared with `cleanupOrphans`) and call it from `src/index.ts` `shutdown()` before exit; plus a startup `resetAllContainerStatusesToStopped()` (right after `cleanupOrphans()`) that clears the phantom `container_status='running'` rows a host-died-without-`close` leaves behind (the `/dashboard` "3 running sessions / 0 containers" symptom — the cache is only ever cleared by the in-process `close` handler, so a restart freezes it). Additive over upstream (upstream relies solely on next-start cleanup) — flag in the commit message for a future manual upstream sync.
