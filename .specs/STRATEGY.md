...
- **OneCLI:** register the `career-pilot-dev` scope; owner OAuth-connects `janedoe.career.dev@gmail.com` via the gated `onecli.dev.hire` UI (or a one-time localhost connect per the caveat); assign the dev Gmail secret to the dev scope only.
...
4. The owner can reach the OneCLI vault UI at the gated `onecli.dev.hire.example.com` (owner-only), has connected `janedoe.career.dev@gmail.com`, and that secret is scoped to `career-pilot-dev` only.
