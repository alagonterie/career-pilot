...
6. Box verification: live `/api/observability` shows non-zero `host`-class spend + real `last_success_age_sec`; `/api/dev/health` returns the report on dev and 404s on a non-dev stack; manual portal pass over the Access-gated dev URL confirms sparklines, lit nodes, and the health panel; an induced failure flips a node to degraded/down honestly. Closes the §24.66 three-dive arc.
