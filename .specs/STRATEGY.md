...
| **9.3** (§24.40) | **The Gmail recruiter-sim** (D2). A dev-only cron + deterministic scenario state machine + Haiku-composed prose, sending from `recruiter.dev@example.com` to the dev candidate inbox (hard recipient allow-list), replying contextually to the candidate agent's outreach, and creating Calendar invites — driving the proactive close-detection / funnel-advancement loop end to end. Injects via the Gmail loop + the CLI/portal channels, never a Telegram bot (§24.40 D11). | 9.2 |
...
#### 24.41 Dev access ergonomics — Tailscale + OneCLI public-URL (next-session kickoff)
