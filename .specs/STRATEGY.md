...
- **D3 — Container→host bridge reuses the existing outbound channel.** The provider yields the lifecycle signal as a dedicated outbound kind (the sandbox trace already crosses container→host as a `kind:'trace'` outbound row via the poll-loop; the owner lifecycle rows take an analogous kind the host routes to `insertProgressRow` rather than the simulator SSE topic). No new transport.
