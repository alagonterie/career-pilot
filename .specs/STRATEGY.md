...
- `GET /api/simulator/:id/stream` — SSE, **named events** `trace | chat | task`; the payload is the parsed outbound row `content`. `trace` = a `TraceEvent` `{ t:'tool'|'subagent'|'result', name?, subagent?, parent_tool_use_id?, input_summary?, cost_usd? }`; `chat` = intermediate assistant text `{ text }`; `task` = the **terminal** message (run complete). No backlog replay (the visitor watches live from connect), no `id:`/seq, no `?since` — unlike the activity stream.
...
- **`lib/use-simulator-run.ts`** — the orchestration hook + state machine: `idle → starting (POST) → running (SSE open) → done (terminal task / stream end) | error (503/unavailable / network)`. Accumulates `TraceEvent[]` for the left pane and the assembled output text for the right pane; surfaces cost/elapsed/cache from the `result` trace. Client-only (SSR renders the input shell).
...
| **9.4** | **Prod cutover + hardening** (the original Phase-9 scope, now gated behind the dev env proving out): persona de-genericization, real `candidate_profile` content + the live `/api/profile` projection, server-side resume PDF, Turnstile + rate-limit on `/contact`, final hardening → then Phase 10 shadow. | 9.1–9.3 |
...
- **24.49e** — Levers 4/5: skills/title-gen + persona lazy-load. **Precursor slice ✅ DONE** — the §24.49d compounding dead-fragment removal (composer skips module fragments whose whole tool surface a group disallows; see the §24.49d bullet for the box measurement). **Lever 4 — investigated + spec'd + building** (see the 24.49e drill-in below): the skills-descriptions block is dropped wholesale by denying the `Skill` tool (owner never invokes a skill; reversible toggle that doesn't foreclose the custom-skills refactor); the title-gen "kill" is the undocumented conversation-summarization-for-`--resume` call, wired as a default-on candidate env under live box-test (low value, may revert). **Lever 5** (persona worked-example lazy-load) remains "large," reopens [[decision_persona_skill_refactor]], edits a runtime artifact → a separate decision-gated effort.
...
**The lever — deny the `Skill` tool.** Skills doc, verbatim: *"Disable all skills by denying the Skill tool."* So adding **`Skill`** to `OWNER_DISALLOWED_TOOLS` removes the WHOLE descriptions block (bundled + CC built-ins) in one entry — the exact §24.49d mechanism, no provider fork. It **subsumes** the `container_configs.skills` allow-list (which can only trim the 8 NanoClaw-bundled; the CC built-ins aren't per-group removable — we can't add `disable-model-invocation` frontmatter to a built-in). **Owner-safe (verified):** the owner never invokes a skill — the persona only *listed* `Skill` (now removed from that list); no subagent palette includes it. Implementation: `Skill` → `OWNER_DISALLOWED_TOOLS`; in the guard test move `Skill` out of `TOOLS_IN_USE` (with a pointer comment); drop `Skill` from the persona's built-ins line. (Considered-and-rejected: narrowing `settingSources` from `['project','user','local']` to drop `user` — too broad; that source also carries `settings.json` env + the OneCLI CA git settings + the skill symlinks. Denying the tool is surgical.)
...
**DoD (24.49e Lever 4).** (1) `Skill` ∈ `OWNER_DISALLOWED_TOOLS`; guard test green with `Skill` out of `TOOLS_IN_USE`; persona no longer lists `Skill`. (2) Box: a fresh owner spawn's request has the skills-descriptions block **absent** and the body shrinks measurably (Portkey `content-length` before/after). (3) Title-gen: a box A/B confirms whether the per-spawn summarization Portkey request disappears under the candidate env — keep + record if yes, revert + record if no. (4) Spec/memory updated with the measured deltas. (5) The reversibility/refactor relationship above is captured so a future session doesn't re-litigate the toggle.
