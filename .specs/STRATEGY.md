...
2. **fetch_source response size still occasionally caps out — this is now the load-bearing fix.** Despite truncation + per-board distribution, ~65KB responses still trigger the SDK's "result too large → save to file" path at the subagent level. The SDK's inline cap for tool results returned to subagents appears tighter (~50-60KB) than for orchestrator tools (~100KB+). The subagent has no `Read` tool in its palette (by design — scrape-jobs is meant to be tightly scoped), so when results spill to file it cannot recover. Result: 0 evaluations, 0 `record_job_lead` calls. **Architectural fix:** redesign so `fetch_source` returns minimal per-posting summaries (title, source, source_job_id, company, location, ~120-char snippet) — the subagent's judgment doesn't need full descriptions to decide "engineering or not." Then `record_job_lead` accepts only `(source, source_job_id)` and the host re-fetches + populates the full payload server-side from a 1h cache keyed on the same tuple. Removes the subagent-as-payload-carrier role entirely. This also resolves issue #1 since the orchestrator stops confabulating when the subagent has real postings to evaluate.
...
**Update 2026-05-27 (post-investigation):** Issue #1 disproven as infrastructure rot — was orchestrator hallucination downstream of issue #2. Investigation captured in the issue #1 entry above. Issue #2 is the real fix path and lands next. Issue #3 (sales-skew test determinism) and the GLM `<Agent>` XML follow-up remain.
