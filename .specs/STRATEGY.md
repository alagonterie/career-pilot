...
- **Structured capture, not prose-parsing.** Today a run accumulates the sandbox's *chat prose* into `simulator_runs.tailored_resume` (unstructured) — the PDF engine needs a `WorkProfile`. So add a read-only sandbox MCP tool **`emit_tailored_resume(profile: WorkProfile)`** — the sandbox analog of the owner's `set_work_profile`, but it NEVER touches `candidate_profile`; it validates + stashes the tailored profile in the run accumulator (→ the run row). The agent calls it once at the end of a run, after `tailor-resume`.
...
- **Cost envelope (D6):** the structured emit adds tokens to a run; re-size the per-run estimate (`simulator_max_budget_usd`) and re-check the daily/per-IP caps so the heavier run can't drain budget — all via `getConfig` (no magic numbers).
