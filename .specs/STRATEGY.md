...
| **9.3** | **The Gmail recruiter-sim** (D2). A dev-only cron + deterministic scenario state machine + Haiku-composed prose, sending from `recruiter.dev@example.com` to the dev candidate inbox (hard recipient allow-list), replying contextually to the candidate agent's outreach, and creating Calendar invites — driving the proactive close-detection / funnel-advancement loop end to end. | 9.2 |
...
- **VM bootstrap (cloud-init + the deploy-backend dev path):** install Docker + pnpm + OneCLI (single gateway, localhost-bound); clone `dev`→`/opt/career-pilot-dev`; write the dev `.env` (`ENVIRONMENT=dev`, `LIVE_MODE=true`, portal port 3002, `TELEGRAM_BOT_TOKEN=<dev>`, OneCLI agent `career-pilot-dev`); migrate the dev checkout's own `data/v2.db`; build the agent image; register `career-pilot-dev.service`.
...
- **OneCLI:** register the `career-pilot-dev` scope; owner OAuth-connects `alagonterie.career.dev@gmail.com` via the gated `onecli.dev.hire` UI (or a one-time localhost connect per the caveat); assign the dev Gmail secret to the dev scope only.
- **Wire + verify:** dev agent groups into the dev service; dev Telegram pairing; then confirm the closed surface end-to-end (DoD below).
- **`deploy-backend.yml` dev path:** push to `dev` → `git pull` + `pnpm install` + restart `career-pilot-dev.service` on the VM (rebuild the image only if `container/` changed). 9.1's `deploy-frontend.yml` already ships the dev frontend.
...
1. A second `career-pilot-dev` systemd service runs on the shared VM, fully isolated from a (future) prod service: own checkout/DB/session tree/CLI socket, own port (3002), own Telegram bot, own OneCLI agent scope + dev Gmail secret, `LIVE_MODE=true`.
...
4. The owner can reach the OneCLI vault UI at the gated `onecli.dev.hire.example.com` (owner-only), has connected `alagonterie.career.dev@gmail.com`, and that secret is scoped to `career-pilot-dev` only.
...
**Spec deltas (this drill-in).** This §24.39 (9.2 drill-in + D6–D10 + the OneCLI-UI owner decision); §24.38's decomposition-table 9.2 row repointed to §24.39; §13 (a pointer to §24.39 for the dev-stack detail); §16 (a note that the *deployed* dev backend extends the local-dev conventions to the VM); Part VI #2 marked resolved-in-9.2 (§24.39, cookie mechanism). Memory: [[project_dev_env_deploy]] (D6–D10 + provided creds), [[status_current]].
