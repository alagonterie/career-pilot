...
- **Fix.** Extract the install-scoped stop loop in `container-runtime.ts` into a shared `stopInstallContainers(reason)` (the `docker ps --filter label=<install> → docker stop` body, returning the count + names); `cleanupOrphans()` delegates to it (start path — behavior/log unchanged). Call it from `src/index.ts shutdown()` after the registered shutdown callbacks and before exit (`host-shutdown` reason) — stopping the money-spending, DB-mutating containers as the first concrete teardown. Bound each `docker stop` with a timeout so a wedged daemon can't push total shutdown past systemd's `TimeoutStopSec` (default 90s) into a SIGKILL that would re-orphan. Label-scoped (not `activeContainers`-scoped) so it reaps even a container the in-memory map lost track of.
- **Division of labor (kept).** Graceful SIGTERM (deploys, `systemctl restart`) → this new shutdown-stop reaps cleanly *now*. Ungraceful exit (SIGKILL/OOM/power loss) → start-time `cleanupOrphans()` stays the backstop. Both share the one primitive. `KillMode=process` stays (intentional — it lets `shutdown()` run to completion; `control-group` wouldn't reach the dockerd-owned containers anyway, so it's not the lever).

**DoD.** On graceful shutdown the host stops its install-labeled containers (no "Unit process (docker) remains running after unit stopped" orphan after a deploy); `cleanupOrphans()` behavior/log unchanged (delegates to the shared fn); each `docker stop` is timeout-bounded. host `tsc` + host unit (`container-runtime.test` extended for `stopInstallContainers` + the shutdown wiring) + prettier green. **Box:** post-deploy the stop log shows the ops container stopped on the way out; the next inbound resumes the ops session from its continuation (time-gated — next deploy + any owner turn). **Spec deltas:** this §24.91; the host-shutdown container-stop is a tracked **addition** over upstream NanoClaw's shutdown (upstream relies solely on next-start `cleanupOrphans`) — flag for `/update-nanoclaw`. Memory: [[todo_backlog]].
