...
- **D2 — `/about` adopts it (owner: full reuse).** `/about` gains a document masthead (title + an "ABOUT · long" register tag) and feeds its ~12 prose sections to `LongformDoc` (`idPrefix="about"`, no stepper — no sealed sections). Section model: a typed `{ id, nav, heading }[]` (short `nav` label for the TOC, full `heading` for the section h2) so the TOC reads cleanly while headings stay sentence-length. The existing `#anonymization` + `#privacy` deep-link ids are preserved as section ids. The live `CostSection` telemetry + the registry-backed `CastRoster` ride along unchanged as section content.
- **D3 — `/experience` adopts it (owner: "same exact treatment").** `WorkSections` already renders a variable set of sections (omit-when-empty); it now also computes the ordered list of *present* sections and renders each marked for the scaffold. `/experience` wraps them in `LongformDoc` (`idPrefix="experience"`, no stepper) below the existing masthead (name + title + provenance + download). The TOC reflects only present sections (a partial profile → a shorter rail, no empty entries).
