...
**Conclusion: the skills-descriptions block is a fixed cost under our `bypassPermissions` setup** — removing it would require not-mounting `/app/skills` (breaks the skill-instruction `@`-import fragments) or forking the provider (the locked anti-pattern). It sits in the 1h-cached prefix, so its ongoing per-turn cost is modest; we stop here. (Considered-and-rejected: narrowing `settingSources` to drop `user` — too broad; that source carries `settings.json` env + OneCLI CA git settings + the skill symlinks.)
...
**DoD (24.49e Lever 4) — partial DONE.** (1) ✅ `Skill` ∈ `OWNER_DISALLOWED_TOOLS`; guard test green with `Skill` out of `TOOLS_IN_USE`; persona no longer lists `Skill`; confirmed absent from a live request's `tools[]`. (2) ❌ The skills-descriptions block is **NOT removable** under our setup — box-tested and CLOSED: `disallowedTools` strips the tool not the block; `container_configs.skills=[]` empties the symlinks but CC discovers the bundled skills from `/app/skills` regardless; the settings `permissions.deny` is bypass-ignored; the CC built-ins are baked in. Fixed cost (mitigated by the 1h cache). Not pursued further. (3) ✅ Title-gen: confirmed gone (one Portkey log post-deploy); flag kept. (4) ✅ Spec/memory updated with the corrected findings. (5) ✅ The reversibility/refactor relationship captured.
