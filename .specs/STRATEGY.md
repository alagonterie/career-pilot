...
- **D3 — Registry of AI *actors*, not just subagents (honesty-load-bearing).** `frontend/src/lib/ai-actors.ts` is the single source of truth: `kind: 'subagent' | 'host' | 'system'`. The six dispatched subagents are `subagent`; **win-confidence and the public-view sanitizer are `host`** — host-side LLM calls that run *outside* the orchestrator loop, so attributing them to a subagent would be a fabrication; `system` ("my agent system") is the orchestrator, the honest fallback for whole-system output (`/work`, the master résumé). Corrected an attribution in the process: the win-confidence score + rationale is host-side (`win-confidence.ts`), **not** `pipeline-scribe` (which authors the published notes).
