...
1. A global `@media (prefers-reduced-motion: reduce)` reset in `app.css` neutralizes all CSS animation + transition (the canonical `*`/`::before`/`::after` reset); the now-redundant `.cp-live-pulse` override is folded into it. Verified: under emulated reduced-motion a loading skeleton's `animate-pulse` is inert (no running animation).
...
**36.5** is drilled in when reached (the §24.35 pattern): meta/OG/favicon/404 (incl. the `NotFound` styling deferred from 36.3) — a drill-in + build with its own DoD.
