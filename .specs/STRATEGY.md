...
- **24.49e** — Levers 4/5: skills/title-gen flag investigation; persona lazy-load decided under [[decision_persona_skill_refactor]] at that point. **Precursor slice ✅ DONE** — the §24.49d compounding dead-fragment removal (composer skips module fragments whose whole tool surface a group disallows; see the §24.49d bullet for the box measurement). **Remaining (not yet started):** Lever 4 is flagged "investigation" — the in-container provider passes `settingSources: ['project','user','local']` (the ~18-skill block hypothesis) and the per-spawn title-gen Haiku call isn't visible in the SDK options, so it needs a research pass → drill-in spec before building; Lever 5 (persona worked-example lazy-load) is "large," reopens [[decision_persona_skill_refactor]], and edits a runtime artifact, so it stays a separate decision-gated effort.
