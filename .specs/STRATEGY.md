...
**Polish (owner local-test pass, 35f).** Three refinements from exercising the modal: (1) the backdrop fades in/out (motion opacity, `AnimatePresence`) rather than snapping; (2) the modal content fades in just as the grow settles (`delay ~0.15s`) and fades out before the box shrinks back — the fade is deliberate, masking the text-stretch a shared-layout grow would otherwise show; (3) the demo panes carry `min-h-[13rem]` (above the tallest synthetic sample) so cycling "show another" doesn't resize the modal.
