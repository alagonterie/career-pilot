...
- **D7 — Provider→status thresholds** (all `getConfig`): `arch_provider_error_rate_degraded` 0.25, `arch_provider_stale_success_sec` 3600 (degraded if last success older); `down` = all recent attempts failed; no rows in window → `idle` (honest: no recent call, no claim).
...
**Config keys (defaults.json):** `portal_observability_cache_ms` 30000, `arch_provider_error_rate_degraded` 0.25, `arch_provider_stale_success_sec` 3600. No new inspector knobs (stable infra thresholds; add later only if a tuning need surfaces).
