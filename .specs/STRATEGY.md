...
- **D3 — DO budget model = pre-charge a fixed per-run estimate on start**, not check-on-start/charge-after (which N concurrent starts can overshoot before any charge lands). `GlobalBudgetCounter` atomically charges `SIM_PER_RUN_ESTIMATE_USD` (= the host's `simulator_max_budget_usd`, 0.10) on each allowed start; daily cap ÷ estimate is the hard runs/day ceiling. Post-hoc actual-cost reconciliation (CLOUDFLARE_PATTERNS §4's backend→Worker `/charge` callback) is a deferred refinement; the pre-charge is the safe ceiling "invincible" wants.
- **D4 — Two config tiers, by layer (the Worker has no DB at the edge).** Edge-tier tunables — the RL burst limits, the DO caps, the Turnstile expected-hostname + secret — are per-environment `wrangler.jsonc` vars/secrets (the Worker can't call `getConfig`). They are deliberately set EQUAL to their host-tier canonicals (`SIM_DAILY_BUDGET_USD` = `sandbox_daily_global_budget_usd` 5; `SIM_PER_IP_DAILY_CAP` = `sandbox_per_ip_daily_run_cap` 10; `SIM_PER_RUN_ESTIMATE_USD` = `simulator_max_budget_usd` 0.10) — each tier enforces at its own layer with its own copy; the wrangler comment cross-references defaults.json. The one host-tier addition (`origin_jwt_validation_enabled`) stays in defaults.json/`getConfig`. Zero-magic-numbers holds across both tiers; the split is just *which* tier owns each value.
...
| 3 | DO global $-budget pre-charge | `$.ts` → `GlobalBudgetCounter` (D3) | total daily spend ($5 ceiling) |
| 4 | DO per-IP daily cap | `$.ts` → `IpDailyCounter` | single-IP grinding (10/day) |
...
**Config (the D4 split).** Edge (`wrangler.jsonc` vars, per env): `TURNSTILE_SECRET`, `TURNSTILE_HOSTNAME`, `SIM_DAILY_BUDGET_USD`, `SIM_PER_RUN_ESTIMATE_USD`, `SIM_PER_IP_DAILY_CAP`; RL limits live in the `unsafe.bindings`. Host (`defaults.json`, NEW): `origin_jwt_validation_enabled` (false locally; true on deployed stacks). Existing host canonicals reused, not duplicated: `sandbox_daily_global_budget_usd`, `sandbox_per_ip_daily_run_cap`, `simulator_max_budget_usd`.
...
2. The two Durable Objects (`IpDailyCounter`, `GlobalBudgetCounter`) reset at midnight UTC via alarms; the global budget pre-charges the per-run estimate (D3).
...
5. Edge tunables are wrangler vars set equal to their defaults.json canonicals (D4); no magic numbers; both suites + 3 tscs + prettier (`--no-semi` frontend) green.
6. **Dev box (application layer):** with test keys auto-passing, hammering the dev `/simulator` trips the per-IP 429 after `SIM_PER_IP_DAILY_CAP`, the global-budget 429 after the daily ceiling, and the Workers-RL 429 on rapid repeats; `wrangler tail` shows the guard decisions; non-gated panels stay live. `terraform apply -var environment=dev` activates the managed ruleset + custom rule; origin-JWT rejects a forged assertion.
