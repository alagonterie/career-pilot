...
- **D6 — Terraform WAF/Bot-Fight posture is environment-parameterized.** Free CF managed ruleset ON for the zone; **Bot Fight Mode ON at the public frontend host, OFF at the api/tunnel host** (it can't be scoped/bypassed and would break the Worker→backend service-token headers — CLOUDFLARE_PATTERNS §6). The single free custom WAF rule + single free rate-limit rule (10 s window) target the frontend host's `/api/simulator` + `/api/contact`. Applied to dev now (harmless behind Access); fully meaningful at prod.

**Layered defense, cheapest-first (the order in `$.ts` for the two guarded mutations).**

| # | Layer | Where | Catches |
|---|---|---|---|
| 0 | Bot Fight Mode + managed WAF | CF edge (Terraform, D6) | obvious bots / known-bad before the Worker runs |
...
3. `infra/waf.tf` adds the managed ruleset + 1 custom WAF rule + 1 rate-limit rule + Bot Fight Mode (frontend host on / api host off), env-parameterized; `terraform plan -var environment=dev` is clean and touches no prod resources.
