...
**Visible-layer follow-up (frontend — out of scope for this pure-backend increment).** The capture writes real `category='turn'` rows and the render path is generic (above), so the lanes light up in production with no frontend change. Two pieces make it *visible in the seeded demo + CI* and stay deferred to a focused follow-up: (1) **dev-seed alignment** — `src/modules/portal/dev/fixtures.ts` still illustrates telemetry on `funnel`/`subagent_progress` rows (a pre-§24.34 demonstration choice); align it to the real shape (telemetry on `category='turn'` rows, NULL on funnel/progress) and re-bless the `home.png` / `live.png` baselines; (2) **`/live` local cost aggregate** — sum `cost_cents` WHERE `category='turn'` into the `/api/telemetry` `local` block so the dashboard shows a real local spend total (host + a frontend lane). Both touch frontend fixtures/baselines, so they are a deliberate separate step.
...
5. The render path is confirmed by inspection: the SSE broadcaster SELECTs every row regardless of category (`WHERE seq > ?`, no category filter), and `LogStream` renders a row's model/tokens/cost/cache/latency lanes generically (falling back to `category` for the label when `agent_name` is null) — so a `category='turn'` row lights up the lanes with **no frontend code change**. Making it *visible in the demo + CI* is the visible-layer follow-up below.
