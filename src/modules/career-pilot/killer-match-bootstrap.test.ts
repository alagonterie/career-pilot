...
function insertSampleTask(opts: { id?: string; status: string; recurrence?: string | null }): void {
  ...
  if (opts.status !== 'pending') {
    inDb
      ...
      .run(opts.status, opts.id ?? 'killer-match');
  }
}
...
describe('hasLiveKillerMatchTask', () => {
  ...
  it('matches by series_id, not just id', () => {
    ...
    inDb
      .prepare("UPDATE messages_in SET series_id = 'killer-match' WHERE id = 'task-clone-1'")
      .run();
    expect(hasLiveKillerMatchTask(inDb)).toBe(true);
  });
...
describe('ensureKillerMatchTask', () => {
  ...
  it('is idempotent — no duplicate insert on second call', () => {
    ...
    const count = (
      inDb
        .prepare("SELECT COUNT(*) AS n FROM messages_in WHERE series_id = 'killer-match'")
        .get() as { n: number }
    ).n;
    expect(count).toBe(1);
  });
  ...
  it('uses custom cron from preferences', () => {
    ...
    const row = inDb
      .prepare("SELECT recurrence FROM messages_in WHERE series_id = 'killer-match'")
