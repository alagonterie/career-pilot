...
/** UTC-fixed compact timestamp (the visit feed reads identically wherever rendered). */
...
function LinkRow({ link }: { link: AdminAttributionLink }) {
  return (
    <tr className="border-t border-border">
      ...
      <td className="py-2 font-mono text-xs text-muted-foreground">{fmtTs(link.lastClickAt)}</td>
    </tr>
  )
}
...
function VisitRow({ visit }: { visit: AdminAttributionVisit }) {
  return (
    <tr className="border-t border-border">
      <td className="py-1.5 pr-4 font-mono text-xs text-muted-foreground">{fmtTs(visit.ts)}</td>
      ...
      <td className="py-1.5 text-muted-foreground">{visit.referrer ?? 'direct'}</td>
    </tr>
  )
}
                ...
                <table className="w-full min-w-[40rem] text-left text-sm">
                  <thead>
                    <tr className="text-[10px] uppercase tracking-widest text-muted-foreground">
                      <th className="px-0 py-2 pr-4 font-mono font-normal">Source</th>
                      ...
                      <th className="py-2 font-mono font-normal">Last click</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.links.map((l) => (
            ...
            {data.recentVisits.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-lg border border-border">
                <table className="w-full min-w-[36rem] text-left text-sm">
                  <thead>
                    <tr className="text-[10px] uppercase tracking-widest text-muted-foreground">
                      <th className="px-0 py-2 pr-4 font-mono font-normal">When (UTC)</th>
                      <th className="py-2 pr-4 font-mono font-normal">Company</th>
                      <th className="py-2 pr-4 font-mono font-normal">Country</th>
                      <th className="py-2 pr-4 font-mono font-normal">Device</th>
                      <th className="py-2 font-mono font-normal">Referrer</th>
                    </tr>
                  </thead>
                  <tbody>
