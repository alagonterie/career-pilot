...
// "Privacy" → /about#privacy (the visitor-privacy disclosure; no standalone page).
//
...
test.describe('site footer — the social/legal strip', () => {
  for (const path of ['/', '/dashboard']) {
    test(`renders on ${path} with the About/Privacy doorways`, async ({ page }) => {
      ...
      // About is /about's second framed entrance; Privacy anchors into /about's
      // visitor-privacy disclosure (no standalone /privacy route).
  ...
  test('Privacy lands on the /about visitor-privacy section', async ({ page }) => {
    await page.goto('/')
